...
int8_t SerialParser::doStartup(){
    ...
    _found = true; //TODO: check for special string
...
void SerialParser::_parseDelimValues(byte &value_ix, byte &ndx)
{
    ...
    switch (value_ix)
        ...
        case 0:
            ...
            if (_rx_cmd == 0)
            { //could be string, instead of "0" cmd
            } 
            break;
        ...
        case 1 ...ARGS_CMDS_SIZE:   //first value start at value_ix 1
            ...
            {
                _rx_vals[_rx_index] = atof(_received_chars); //convert to float
            }
            _rx_index++;
            break;
