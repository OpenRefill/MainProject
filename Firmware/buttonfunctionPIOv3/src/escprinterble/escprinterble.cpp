...
/*
looks like it based on esspressif example, see here:
https://dronebotworkshop.com/esp32-bluetooth/

https://github.com/nkolban/esp32-snippets/issues/874
https://github.com/nkolban/esp32-snippets/issues/943
https://github.com/nkolban/esp32-snippets/blob/master/cpp_utils/BLEScan.h#L52
...
// status
static bool ECS_CONNECTED = false;
...
class MyClientCallback : public BLEClientCallbacks
{
  ...
  {
    ECS_CONNECTED = false;
    Serial.printf("BLEClient: onDisconnect, connected:%d\n",pclient->isConnected());
    // pclient->disconnect();
  }
};
...
class MyAdvertisedDeviceCallbacks : public BLEAdvertisedDeviceCallbacks
{
  ...
  {
    Serial.printf("BLEAdvertisedDevice: found, add:%s\n", advertisedDevice.toString().c_str());
...
{
  Serial.printf("ESC-BLE:do disconnect,connected:%d\n",isConnected());
...
bool EscPos::connect(void)
{
  ...
  if (myDevice == nullptr)
  {
    Serial.println("ESC-BLE:Failed to find service UUID");
    return false;
  }

  Serial.printf("ESC-BLE:Forming a connection, addr:%s\n",myDevice->getAddress().toString().c_str());

  pClient = BLEDevice::createClient();
  Serial.println("ESC-BLE: created client");

  pClient->setClientCallbacks(new MyClientCallback());

  // Connect to the remote BLE Server.
  pClient->connect(myDevice); // if you pass BLEAdvertisedDevice instead of address, it will be recognized type of peer device address (public or private)
  Serial.println("ESC-BLE: Connected to server");
  ...
  {
    Serial.printf("ESC-BLE: Failed to find service, UUID:%s\n",serviceUUID.toString().c_str());
    pClient->disconnect();
    return false;
  }
  Serial.println("ESC-BLE: Found service");

  // Obtain a reference to the characteristic in the service of the remote BLE server.
  pRemoteCharacteristic = pRemoteService->getCharacteristic(charUUID);
  if (pRemoteCharacteristic == nullptr)
  {
    Serial.printf("ESC-BLE:Failed to find our characteristic, UUID:%s\n",charUUID.toString().c_str());
    pClient->disconnect();
    return false;
  }
  Serial.println("ESC-BLE: Found characteristic");

  // Read the value of the characteristic.
  if (pRemoteCharacteristic->canRead())
  {
    std::string value = pRemoteCharacteristic->readValue();
    Serial.printf("ESC-BLE: characteristic value: %s\n",value.c_str());
  }
...
void EscPos::connectLoop(void)
{
  if (doConnect == true)
  {
    ...
    {
      Serial.println("ESC-BLE:Failed to connect the server");
    }
    doConnect = false;
  }
  ...
  if (!isConnected() && doScan)
  {
    BLEScan *pBLEScan = BLEDevice::getScan();
    // pBLEScan->erase();
    pBLEScan->start(0); // this is just example to start scan after disconnect, most likely there is better way to do it in arduino
    delay(100);
  }
...
void EscPos::start(const char *name)
{

  Serial.printf("ESC-BLE: Start Client, name:%s\n",name);
  BLEDevice::init(name);
