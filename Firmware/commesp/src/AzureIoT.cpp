...
azure_iot_client_state_t azure_iot_get_iot_state(azure_iot_t* azure_iot)
{
  _az_PRECONDITION_NOT_NULL(azure_iot);
  return azure_iot->state;
}
...
void azure_iot_do_work(azure_iot_t *azure_iot)
{
  ...
  case azure_iot_state_connected_to_dps:
    ...
    {
      azure_iot->state = azure_iot_state_error;
      LogError("Failed subscribing to Azure Device Provisioning response topic.");
      return;
    }
  ...
  case azure_iot_state_provisioned:
    ...
    LogInfo("Disconnecting provisioning service...");
    if (azure_iot->config->use_device_provisioning &&
        azure_iot->mqtt_client_handle != NULL &&
...
int azure_iot_send_telemetry(azure_iot_t *azure_iot, az_span message, bool is_dispense)
{
  _az_PRECONDITION_NOT_NULL(azure_iot);
  ...
  if (is_dispense)  mqtt_message.qos = mqtt_qos_at_least_once;  // 1
  else              mqtt_message.qos = mqtt_qos_at_most_once;   // 0

  int packet_id = azure_iot->config->mqtt_client_interface.mqtt_client_publish(azure_iot->mqtt_client_handle, &mqtt_message);
  Serial.printf("PUBLISH-SENT, id:%d\n",packet_id);
  if (is_dispense)
  {
    char c_msg[MSG_NUM_CHARS] = "";
    az_span_to_str(c_msg,MSG_NUM_CHARS,message);
    if (packet_id < 0)  dxOutbox.storeMsg(c_msg);
    else                dxOutbox.enqueueMsg(c_msg,packet_id,true,-1,true);
  }
...
int azure_iot_resend_telemetry(azure_iot_t *azure_iot)
{
  ...
  mqtt_message_t mqtt_message;
  _az_PRECONDITION_NOT_NULL(azure_iot);
  if (time_millis - last_resend < 1000) return 0;
  last_resend = time_millis;
  // only re-publish one at the time
  if (!dxOutbox.isEnqueueBoxEmpty()) return 0;
    
  int nvm_msg_id = dxOutbox.getNextUnsentId();
  if (nvm_msg_id == -1) return 0;

  Serial.printf("resend next,nvm_msg_id:%d\n",nvm_msg_id);
  
  char msg[MSG_NUM_CHARS] = "";
  dxOutbox.getMsg(nvm_msg_id,msg);
  size_t len = strlen(msg);
  if (len <= 2)
  {
      LogError("resend msg empty,len:%d,msg:%s\n",len,msg);
      return -1;
  }

  az_span msg_span = az_span_create_from_str(msg);
  _az_PRECONDITION_VALID_SPAN(msg_span, 1, false);
  ...
  mqtt_message.topic = az_span_slice(azure_iot->data_buffer, 0, topic_length + 1);
  mqtt_message.payload = msg_span;
  mqtt_message.qos = mqtt_qos_at_least_once;  // 1
  
  // sending
  int packet_id = azure_iot->config->mqtt_client_interface.mqtt_client_publish(azure_iot->mqtt_client_handle, &mqtt_message);
  LogInfo("PUBLISH-RESENT, id:%d\n",packet_id);
  
  // enque if sent ok 
  if  (packet_id >= 0) dxOutbox.enqueueMsg(msg,packet_id,true,nvm_msg_id,true);
  
  EXIT_IF_TRUE(packet_id < 0, RESULT_ERROR, "Failed prepublishing to telemetry topic");
...
int azure_iot_mqtt_client_connected(azure_iot_t *azure_iot)
{
  ...
  {
    LogInfo("Connected to Hub");
    azure_iot->state = azure_iot_state_connected_to_hub;
    result = RESULT_OK;
  }
  else if (azure_iot->state == azure_iot_state_connected_to_hub)
  {
    LogInfo("Already connected to Hub -- error?");
    result = RESULT_OK;
  }
  else
  {
    LogError("Unexpected mqtt client connection, iot-state:%d", azure_iot->state);
    azure_iot->state = azure_iot_state_error;
    result = RESULT_ERROR;
  }
      ...
      else if (register_response.operation_status == AZ_IOT_PROVISIONING_STATUS_ASSIGNED)
      {
        ...
        else
        {
          ...
          else
          {
            ...
            azure_iot->state = azure_iot_state_provisioned;
            LogInfo("Device provisioning success");
            result = RESULT_OK;
          }
        }
      }
